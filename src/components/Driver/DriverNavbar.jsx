...
const DriverNavbar = () => {
  ...
  const [isTracking, setIsTracking] = useState(false); // Track location sharing state
  ...
  const handleLogout = () => {
    if (window.confirm("Are you sure you want to logout?")) {
      setIsTracking(false); // Stop tracking on logout
  ...
  const handleGoToLocation = () => {
    if (isTracking) {
      // If tracking is active, ask to stop
      if (window.confirm("Location access is allowed. Are you sure you want to cancel location access?")) {
        setIsTracking(false); // Stop tracking
      }
    } else {
      // If tracking is not active, ask to start
      if (window.confirm("Are you sure you want to send the location?")) {
  ...
  return (
    <>
      <DriverTracker startTracking={isTracking} stopTracking={!isTracking} />
      ...
      >
        <div className="flex items-center justify-between p-4 lg:px-8 w-full">
          ...
          <div className="hidden lg:flex items-center space-x-4">
            ...
            <button
              ...
              className="flex items-center space-x-2 p-2 cursor-pointer font-medium hover:text-red-500 transition-all font-[Poppins,Arial,Verdana,Tahoma,sans-serif]"
            >
            ...
            >
              ...
              <span className="text-base">
                {isTracking ? "Stop Location Access" : "Location Access"}
              </span>
            </button>

            <div className="flex items-center space-x-2 font-[Poppins,Arial,Verdana,Tahoma,sans-serif]">
        ...
        {isOpen && (
          <div className="lg:hidden absolute top-full left-0 w-full backdrop-blur-lg p-4 flex flex-col space-y-4 shadow-md">
            <div className="flex items-center">
              <span className="mr-2 font-[Poppins,Arial,Verdana,Tahoma,sans-serif]">
                Theme:
              </span>
              <ThemeToggleButton />
            </div>
            ...
            <button
              onClick={handleGoHome}
              className="flex items-center space-x-2 p-2 w-full text-left hover:text-red-500 font-[Poppins,Arial,Verdana,Tahoma,sans-serif]"
            >
              <FaHome className="w-5 h-5" />
            ...
            <div className="flex items-center space-x-2 p-2 font-[Poppins,Arial,Verdana,Tahoma,sans-serif]">
              <FaUser className="w-5 h-5" />
              <span className="text-base font-medium">{username}</span>
            </div>
